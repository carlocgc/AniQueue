...
/// </summary>
/// <remarks>
/// A malformed <c>userconfig.json</c> must not be fatal. The configuration provider
/// throws while the host is being built, before logging exists, so a missing comma
/// would otherwise produce a stack trace on the console and no application — the
/// worst possible behaviour for the one file an operator edits by hand.
///
/// So the file is allowed to fail. What it configures is skipped, everything else
/// starts normally, and this records why for the banner that says so.
/// </remarks>
public sealed class UserConfigStatus
{
    ...
    /// </summary>
    /// <remarks>
    /// Called before AniQueue reloads configuration it has just written itself.
