...
/// is worth retrying smaller, and a model nothing answered at is not.
/// </remarks>
public enum ScoringEndpointFailure
{
    ...
    /// <summary>The address was refused before anything was sent.</summary>
    AddressRefused,
...
public sealed record ScoringEndpointResult
{
    ...
    /// </summary>
    /// <remarks>
    /// Bounded and body-only. It is what a person debugging their own server
...
/// </summary>
/// <remarks>
/// It carries; it does not decide. What is sent is what
/// <see cref="ScoringPromptBuilder"/> and <see cref="ScoringRequestWriter"/> produce
/// for the manual route, byte for byte, and what comes back goes to the same parser.
/// Nothing here knows what a good ranking looks like.
///
/// It never throws for a failure that is the endpoint's. A self-hosted model being
/// switched off is the normal state of a fresh install, so an unreachable address, a
/// timeout and a rejected request all come back as results with a reason on them.
/// </remarks>
